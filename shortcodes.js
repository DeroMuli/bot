...
⌨️ Reply with exit to go back`, num)
}
...
function shortresponse (req) {
    ...
    if (test.includes("teamfixtures")) {
        ...
        grouptables(qs, num).then(() => {
            sendmessage("⌨️ Reply with another short code or exit to go back 🔙", num);
            DBNAME[num] = "Short";
...
function getteamfixture(teamname,num) {
    ...
    getteamfixtures(teamname, qs, num).then(ans => {
...
function getteamresult(teamname,num) {
    ...
    getteamresults(teamname, qs, num).then(ans => {
        ans += "⌨️ Reply with another short code or exit to go back 🔙";
        sendmessage(ans, num);
    }).catch(e => { console.log(e) });
}
...
function gettopscorers(id, num) {
    ...
    leaguetopscorers(qs, num).then(ans => {
        ans += "⌨️ Reply with another short code or exit to go back 🔙";
        sendmessage(ans, num);
    }).catch(e => { console.log(e) });
}
...
function getparameter(numofcommand,spaced) {
    ...
    array = array.map(word => { return word.trim() });
