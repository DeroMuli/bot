...
async function getonlineteamstats(teamname, qs, num) {
  ...
  if (response[0]) {
    ...
    for (let lineupz of lineups) {
      if (lineupz.played > lineup.played) lineup = lineupz;
    }
    let ans = "";
    ans += ` ${teamname} stats 📈 📊`;
    ans += "\n\n";
...
async function getonlineplayerstats(qs, num) {
  ...
  if (playerprofile) {
    ...
    let redcards =
      playerprofile.statistics[0].cards.red +
      playerprofile.statistics[0].cards.yellowred;
